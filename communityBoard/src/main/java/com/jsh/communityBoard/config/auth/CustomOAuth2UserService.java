...
import com.jsh.communityBoard.config.auth.dto.OAuthAttributes;
...
/*
 * 로그인 이후 가져온 사용자의 정보(email, name, picture)를 이용해서 가입 및 정보수정, 세션 저장
 */

// OAuth2UserService<OAuth2UserRequest, OAuth2User>를 구현한 클래스
@RequiredArgsConstructor
@Service
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    /* DI */
    private final UserRepository userRepository; // 회원 저장 DB
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException{
        ...
        String registrationId = userRequest.getClientRegistration().getRegistrationId(); // 로그인 진행 중인 서비스 구분코드
